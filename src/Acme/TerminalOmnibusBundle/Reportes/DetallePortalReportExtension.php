...
class DetallePortalReportExtension extends \ReportExtension{
    
    public $container = null;
    public $reportFileName = "detallePortal";
    public $alias = "detallePortal";
    public $enabled = true;
    public function getParam(){
        ...
        if($this->container !== null){
            ...
            $command = $request->get('detallePortal');
               
            $portal = $command["portal"];
            if($portal === null || trim($portal) === ""){
                throw new RuntimeException("m1Debe seleccionar un portal web.");
            }
            $parameters->put('DATA_PORTAL_ID', intval(trim($portal)));
            ...
            if($rangoFecha !== null && trim($rangoFecha) !== ""){
                ...
                if(count($rangoFechaArray) === 2){
                    ...
                    if($fechaInicialStr !== "" && $fechaFinalStr !== ""){
                        ...
                        throw new RuntimeException("m1Rango de fecha invalido");
                    }                    
                }else{
                    throw new RuntimeException("m1Rango de fecha invalido");
                }
