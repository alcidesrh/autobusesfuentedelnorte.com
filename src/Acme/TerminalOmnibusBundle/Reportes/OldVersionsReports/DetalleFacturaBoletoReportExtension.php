...
class DetalleFacturaBoletoReportExtension extends \ReportExtension{
    
    public $container = null;
    public $reportFileName = "detalleFacturaBoleto";
    public $alias = "detalleFacturaBoleto";
    public $enabled = true;
    public function getParam(){
        ...
        if($this->container !== null){
            ...
            $command = $request->get('detalleFactura');
            ...
            $autorizacionTarjeta = $command["autorizacionTarjeta"];
            if($autorizacionTarjeta !== null && trim($autorizacionTarjeta) !== ""){
                $parameters->put('DATA_AUTORIZACION_TARJETA', $autorizacionTarjeta);
            }
            ...
            if(isset($command["mostrarAutorizacionTarjeta"]) && $command["mostrarAutorizacionTarjeta"] === "1"){
                $parameters->put('DATA_MOSTRAR_SOLO_TARJETAS', strval(true));
            }else{
                $parameters->put('DATA_MOSTRAR_SOLO_TARJETAS', strval(false));
            }
            
            if(isset($command["mostrarReferenciaExterna"]) && $command["mostrarReferenciaExterna"] === "1"){
                $parameters->put('DATA_MOSTRAR_SOLO_REFERENCIAS_EXTERNAS', strval(true));
            }else{
                $parameters->put('DATA_MOSTRAR_SOLO_REFERENCIAS_EXTERNAS', strval(false));
            }
