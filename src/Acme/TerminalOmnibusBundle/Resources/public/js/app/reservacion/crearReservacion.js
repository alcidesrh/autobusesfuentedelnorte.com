...
//	console.debug("crearReservacion.funcionesAddOnload-end");
    ...
    _init : function() {
        ...
        var pathlistarclientespaginando = $("#clienteBase").data("pathlistarclientespaginando");
         $("#clienteBase").select2({
...
//                console.debug(confirmed);
         ...
         $("#crear_reservacion_command_fechaSalida").on("change", crearReservacion.changeFiltersSalida);
         $("#crear_reservacion_command_estacionOrigen").on("change", crearReservacion.changeFiltersSalida);
...
//        console.debug($(this));
        ...
        var listaClienteReservacion = $("#crear_reservacion_command_listaClienteReservacion").val();
        if(listaClienteReservacion){ listaClienteReservacion = JSON.parse(listaClienteReservacion); }
        else{ listaClienteReservacion = []; }
        ...
        if(item === null){
           ...
           listaClienteReservacion = core.removeItemArray(listaClienteReservacion, item);
           figure.removeClass("selected");
           crearReservacion.renderItemGridDeletedReservacion(item, listaClienteReservacion);
        }
//        console.debug(listaClienteReservacion);
        $("#crear_reservacion_command_listaClienteReservacion").val(JSON.stringify(listaClienteReservacion));
    },
...
//        console.debug("clienteActionsHidden...");
        ...
        clienteActionsHidden.find("#updateCliente").click(crearReservacion.updateCliente);
        clienteActionsHidden.find("#updateCliente").data("index", id);
        clienteActionsHidden.find("#seachCliente").click(crearReservacion.seachCliente);
...
//        console.debug(inputSelect);
    ...
    renderItemGridDeletedReservacion: function(item, listaClienteReservacion) {
        if(!listaClienteReservacion){
            listaClienteReservacion = $("#crear_reservacion_command_listaClienteReservacion").val();
            if(listaClienteReservacion){ listaClienteReservacion = JSON.parse(listaClienteReservacion); }
            else{ listaClienteReservacion = []; }
        }
        
        if(listaClienteReservacion.length === 0){
            $("#clienteReservacionBody").find("tr").not("#clienteReservacionVacioTR").remove(); //Elimino todos los tr
            ...
            var numero = item.numero;
            $("#clienteReservacionBody").find(".trSelect"+numero).remove();
        }
    ...
    clearItemGridReservaciones: function() {
        $("#crear_reservacion_command_listaClienteReservacion").val(JSON.stringify([]));
        $("#clienteReservacionBody").find("tr").not("#clienteReservacionVacioTR").remove(); //Elimino todos los tr
        $("#clienteReservacionBody").find("#clienteReservacionVacioTR").show(); //Muestro el vacio
    },
...
//        console.log("imprimirDocumentos-end");
    ...
    syncronizarListaReservaciones: function() {
        ...
        $.each(listaClienteReservacion, function(){
            this.idCliente = $(".trSelect"+this.numero).find('input[id^="cliente_reservacion"]').val();
        });
//        console.debug(listaClienteReservacion);
        $("#crear_reservacion_command_listaClienteReservacion").val(JSON.stringify(listaClienteReservacion));
    },
...
//        console.debug($(this));
        ...
        if(core.customValidateForm(reservacionForm) === true && crearReservacion.customValidate() === true){
...
//                        console.log("procesarRespuestaServidor....okook");
    ...
    customValidate: function(dialog) {
        ...
        cantidadReservaciones = $("#clienteReservacionBody").find("tr").not("#clienteReservacionVacioTR").length;
...
//        console.log("addCliente -- init ...."); 
        var inputCliente = $(this).parent().parent().find("input[id^='cliente_reservacion']");
        frondend.loadSubPage(e, $(this), function(id) {
            if (id !== "") {
                core.request({
