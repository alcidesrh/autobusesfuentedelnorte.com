...
class EncomiendaAnuladoUsuarioType extends AbstractType{
    ...
    {  
        ...
        $builder->add('usuario', 'entity', array(
            ...
            'required' => false,
            'attr' =>  array('class' => 'span8'),
            'query_builder' => function(EntityRepository $er) use ($keyEmpresasUsuario, $estacionUsuario)  {
                $query = $er->createQueryBuilder('u');
                $query->leftJoin("u.empresas", "em");
                $query->andWhere('em.id IN (:keyEmpresasUsuario)');
                $query->setParameter("keyEmpresasUsuario", $keyEmpresasUsuario);
                if($estacionUsuario !== null){
                    $query->leftJoin("u.estacion", "es");
                    $query->andWhere('es.id is null or es.id=:estacionUsuario');
                    $query->setParameter("estacionUsuario", $estacionUsuario->getId());
                }
                return $query;
            }
    ...
    {
        return 'encomiendaAnuladoUsuario';
    }
