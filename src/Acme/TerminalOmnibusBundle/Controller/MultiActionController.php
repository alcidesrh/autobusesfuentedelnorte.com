...
//                        $WSResultadoReceptor = $result['ObtenerIdReceptorResult']['WSDescripcion'];
                        
//                        return UtilService::returnError($this, "m1Nit no encontrado. ".$WSResultadoReceptor. ". TRANSPORTES FDN.");
    ...
    */
    public function getRutasAlternas(Request $request) {
        ...
        try {
            ...
            if( $idEstacionOrigen !== null && is_numeric($idEstacionOrigen) && $idEstacionDestino !== null && is_numeric($idEstacionDestino) &&
                ...
                if($encomienda !== null){
                    ...
                    foreach ($rutas as $ruta) {
                        ...
                        if($empresaRuta !== null)
                        {
                            if($empresaEncomienda === $empresaRuta || 
                              ((
...
//            echo $exc->getTraceAsString();
    ...
    */
    public function getTarifaEncomiendaTopMenu(Request $request) {
        ...
        $estacionOrigen = $request->query->get('estacionOrigen');
        if (is_null($estacionOrigen)) { $estacionOrigen = $request->request->get('estacionOrigen'); }
        $estacionDestino = $request->query->get('estacionDestino');
        if (is_null($estacionDestino)) { $estacionDestino = $request->request->get('estacionDestino'); }
        $tipoEncomienda = $request->query->get('tipoEncomienda');
        if (is_null($tipoEncomienda)) { $tipoEncomienda = $request->request->get('tipoEncomienda'); }
        $cantidad = $request->query->get('cantidad');
        if (is_null($cantidad)) { $cantidad = $request->request->get('cantidad'); }
        $tipoEncomiendaEspecial = $request->query->get('tipoEncomiendaEspecial');
        if (is_null($tipoEncomiendaEspecial)) { $tipoEncomiendaEspecial = $request->request->get('tipoEncomiendaEspecial'); }
        $peso = $request->query->get('peso');
        if (is_null($peso)) { $peso = $request->request->get('peso'); }
        $alto = $request->query->get('alto');
        if (is_null($alto)) { $alto = $request->request->get('alto'); }
        $ancho = $request->query->get('ancho');
        if (is_null($ancho)) { $ancho = $request->request->get('ancho'); }
        $profundidad = $request->query->get('profundidad');
        if (is_null($profundidad)) { $profundidad = $request->request->get('profundidad'); }
    ...
    */
    public function listarFacturas2(Request $request) {
        ...
        if($estacion !== null && trim($estacion) !== ""){
            try {
                $facturas = array();
                $facturas = $this->getDoctrine()->getManager()->getRepository('AcmeTerminalOmnibusBundle:Factura')->getFacturasPorEstacion($estacion, 
        ...
        if($idSalida !== null && trim($idSalida) !== ""){
            ...
            try {
                ...
                if($salida !== null){
                    ...
                    $items = $this->getDoctrine()->getRepository('AcmeTerminalOmnibusBundle:Encomienda')->
                                listarEncomiendasEmbarcadasBySalidaParaProcesar($idSalida);
                    foreach ($items as $item) {
...
//                                "cliente" => $item->getClienteRemitente()->getNombre()
                    ...
                    $items = $this->getDoctrine()->getRepository('AcmeTerminalOmnibusBundle:Encomienda')->
                                listarEncomiendasPendientesByRutaParaProcesar($salida->getEmpresa(), $salida->getItinerario()->getRuta(), $estacionUsuario);
                    foreach ($items as $item) {
                        if($item instanceof Encomienda){
                            $puedeModificar = false;
                            if($estacionUsuario == null){
        ...
        if($idEstacion !== null && trim($idEstacion) !== ""){
            ...
            try {
                ...
                if($estacion !== null){
                    ...
                    $items = $this->getDoctrine()->getRepository('AcmeTerminalOmnibusBundle:Encomienda')->
                                listarEncomiendasPendientesPorEstacion($estacion);
        ...
        if($idEstacion !== null && trim($idEstacion) !== "" && $idEmpresa !== null && trim($idEmpresa) !== ""){
            ...
            try {
                ...
                if($estacion !== null){
                    ...
                    $items = $this->getDoctrine()->getRepository('AcmeTerminalOmnibusBundle:Encomienda')->
                                listarEncomiendasPendientesEntrega($estacion, $idEmpresa, $this->getUser());
    ...
    */
    public function listarTipoEncomiendaEspecialPaginando(Request $request) {
        ...
        try {
            ...
            $items = $this->getDoctrine()->getRepository('AcmeTerminalOmnibusBundle:TipoEncomiendaEspeciales')->
                    listarTipoEncomiendaEspecialesPaginandoNativo($pageLimit, $term, $id);
            foreach ($items as $item) {
