...
class Scheduler implements ContainerAwareInterface
{
    ...
    protected function processTags(Job $job)
    {
        ...
        if ($collection->count()) {
            ...
            $qb = $this->container->get('doctrine')->getManager()->getRepository('AcmeBackendBundle:JobTag')->createQueryBuilder('t');
        ...
        foreach($this->getJobRepository()->getExecutableJobs() as $job) {
            ...
            while ($check) {
                try {
                    ...
                    $emailsAdmin = $this->getManager()->getRepository('AcmeBackendBundle:User')->findEmailSuperAdmin();
    ...
    {
        return $this->getManager()->getRepository('AcmeBackendBundle:Job');
    }
