...
class UtilService {
   ...
   protected $container;
    ...
    public static function getRefereciaExternaReservacion($idWeb){
        $idWeb = strval($idWeb);
        return $idWeb === "1" ? "INTERNET_PIONERA" : ($idWeb === "2" ? "INTERNET_MITOCHA" : ("WEB_NRO_" . $idWeb));
    }
...
//            var_dump($error);
    ...
    public static function calcularPrecioTotalReasignado(Boleto $boleto, $moneda = null){
        $monedaBoleto = $boleto->getMoneda();
        if($moneda !== null && $moneda !== $monedaBoleto){
            throw new \RuntimeException("Las monedas de pago de los boletos son diferentes.");
        }
        $importe = $boleto->getPrecioCalculado();
        if($boleto->getReasignado() !== null){
            $importe += UtilService::calcularPrecioTotalReasignado($boleto->getReasignado(), $monedaBoleto);
        }
        return $importe;
    }
    public static function calcularPrecioTotalReasignadoMonedaBase(Boleto $boleto){
        $importe = $boleto->getPrecioCalculadoMonedaBase();
        if($boleto->getReasignado() !== null){
            $importe += UtilService::calcularPrecioTotalReasignadoMonedaBase($boleto->getReasignado());
        }
        return $importe;
    }
    public static function calcularTarifaAdicionalTotalReasignadoMonedaBase(Boleto $boleto){
        $importe = $boleto->getTarifaAdicionalMonedaBase();
        if($boleto->getReasignado() !== null){
            $importe += UtilService::calcularTarifaAdicionalTotalReasignadoMonedaBase($boleto->getReasignado());
        }
        return $importe;
    }
    public static function getLastBoletoReasignado(Boleto $boleto, $context){
        if($boleto->getEstado()->getId() === EstadoBoleto::REASIGNADO){
            return $context->get('service_container')->get("doctrine")->getRepository('AcmeTerminalOmnibusBundle:Boleto')->getLastBoletoReasignado($boleto);
        }
        return $boleto;
    }
    public static function getBoletosReasignados(Boleto $boleto){
        $boletos = array();
        if($boleto->getReasignado() !== null){
            $boletos[] = $boleto->getReasignado();
            $boletos = array_merge($boletos, UtilService::getBoletosReasignados($boleto->getReasignado()));
        }
        return $boletos;
    }
    public static function getBoletosFacturasBoletosReasignados(Boleto $boleto){
        $facturas = array();
        if($boleto->getFacturaGenerada() != null){
            $facturas[] = $boleto->getFacturaGenerada();
        }
        if($boleto->getReasignado() !== null){
            $facturas = array_merge($facturas, UtilService::getBoletosFacturasBoletosReasignados($boleto->getReasignado()));
        }
        return $facturas;
    }
    public static function getBoletosVoucherAgenciaBoletosReasignados(Boleto $boleto){
        $vouchersAgencia = array();
        if($boleto->getVoucherAgencia() != null){
            $vouchersAgencia[] = $boleto->getVoucherAgencia();
        }
        if($boleto->getReasignado() !== null){
            $vouchersAgencia = array_merge($vouchersAgencia, UtilService::getBoletosVoucherAgenciaBoletosReasignados($boleto->getReasignado()));
        }
        return $vouchersAgencia;
    }
...
//                var_dump("chequeando ip:" . $ip . ", en la red:" . $network);
    ...
    public static  function returnSuccess($context, $options = array()) {
        ...
        return $context->render('AcmeTerminalOmnibusBundle:Commun:respuestaServidor.html.twig', $options);
    }
    
    public static  function checkError($textError) {
        if($textError === null || trim($textError) === ""){
           $textError = "m1Ha ocurrido un error en el sistema";
        }else{
            
            if(strpos($textError, "CUSTOM_IDX_BOLETO_SALIDA_ASIENTO_ESTADO") !== false){
                $textError = "m1El asiento acaba de ser ocupado, seleccione otro asiento.";
            }else if(strpos($textError, "CUSTOM_IDX_RESERVACION_SALIDA_ASIENTO_ESTADO") !== false){
                $textError = "m1El asiento acaba de ser ocupado, seleccione otro asiento.";
            }
        }
        return $textError;
    ...
    public static  function returnError($context, $textError = null) {
       ...
       return $context->render('AcmeTerminalOmnibusBundle:Commun:respuestaServidor.html.twig', array(
